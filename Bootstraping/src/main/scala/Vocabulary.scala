...
object Vocabulary {
  ...
  def FILENAME = "Vocabulary"
  def K = "sex"
  def V = "vocabulary"
  def TIMES = 50

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("Vocabulary").master("local[*]").getOrCreate()
    val csv = spark.sparkContext.textFile("Vocab.csv")
