...
object Unemployment {
  ...
  def V = "tenure"
  def TIMES = 10
  def SAMPLE_RATE = 0.25

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().appName("Benefits").master("local[*]").getOrCreate()
    val csv = spark.sparkContext.textFile("Benefits.csv")
    ...
    val carTypes = mtcdata.map(p => Employee(p(9), p(5))).toDF.cache()
        
    carTypes.select(K,V).show()
